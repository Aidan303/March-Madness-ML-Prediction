...
def build_model(calibration: str):
    ...
    if calibration in {"sigmoid", "isotonic"}:
        return CalibratedClassifierCV(
            estimator=logistic_baseline_pipeline(), method=calibration, cv=5
        )
...
def main() -> None:
    ...
    if len(bracket_preds) > 0:
        out_brk = OUT_DIR / f"{target_season}_bracket_predictions.csv"
    ...
    if len(champion_preds) > 0:
        out_chmp = OUT_DIR / f"{target_season}_champion_predictions.csv"
